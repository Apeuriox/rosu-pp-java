//! Adapter for the unmodified crates.io rosu-pp 3.1.0 release.
...
use rosu_mods_202502::{GameMod, GameMode as ModsGameMode, serde::GameModsSeed};
use rosu_pp_202502 as rosu;
use serde::de::{DeserializeSeed, IntoDeserializer};

pub static ALGORITHM: Algorithm = Algorithm {
    id: backend_api::ALGORITHM_202502,
    name: "REWORK_202502",
    version: "rework-202502-rosu-pp-3.1.0",
    details: "Unmodified crates.io rosu-pp 3.1.0",
    capabilities: capability::ALL_BASE,
};

pub struct Backend202502;

impl Backend for Backend202502 {
...
impl BackendBeatmap for Map202502 {
    fn algorithm(&self) -> &'static Algorithm {
        &ALGORITHM
    }
...
fn validate_performance(request: &PerformanceRequest, mode: Mode) -> Result<(), BackendError> {
    ...
    if fields & score_field::LEGACY_TOTAL_SCORE != 0 {
        return Err(BackendError::UnsupportedOption(
            "legacy total score is not supported by rosu-pp 3.1.0".into(),
        ));
    }
    if fields
...
mod tests {
    ...
    fn metadata_is_stable() {
        assert_eq!(ALGORITHM.id, 20_250_200);
        assert_eq!(ALGORITHM.version, "rework-202502-rosu-pp-3.1.0");
