//! Adapter for the unmodified crates.io rosu-pp 2.0.0 release.
...
use rosu_mods_202411::{GameMod, GameMode as ModsGameMode, serde::GameModsSeed};
use rosu_pp_202411 as rosu;
use serde::de::{DeserializeSeed, IntoDeserializer};

pub static ALGORITHM: Algorithm = Algorithm {
    id: backend_api::ALGORITHM_202411,
    name: "REWORK_202411",
    version: "rework-202411-rosu-pp-2.0.0",
    details: "Unmodified crates.io rosu-pp 2.0.0",
    capabilities: capability::ALL_BASE,
};

pub struct Backend202411;

impl Backend for Backend202411 {
...
impl BackendBeatmap for Map202411 {
    fn algorithm(&self) -> &'static Algorithm {
        &ALGORITHM
    }
...
fn parse_mods(input: &ModInput, mode: Mode) -> Result<GameMods, BackendError> {
    ...
    let structured = GameModsSeed::Mode(mods_mode)
...
fn validate_performance(request: &PerformanceRequest, mode: Mode) -> Result<(), BackendError> {
    ...
    if fields & score_field::LEGACY_TOTAL_SCORE != 0 {
        return Err(BackendError::UnsupportedOption(
            "legacy total score is not supported by rosu-pp 2.0.0".into(),
        ));
    }
    if fields
...
fn validate_score_state(state: &ScoreState) -> Result<(), BackendError> {
    if state.legacy_total_score.is_some() {
        return Err(BackendError::UnsupportedOption(
            "legacy total score is not supported by rosu-pp 2.0.0".into(),
        ));
    }
...
mod tests {
    ...
    fn metadata_is_stable() {
        assert_eq!(ALGORITHM.id, 20_241_100);
        assert_eq!(ALGORITHM.version, "rework-202411-rosu-pp-2.0.0");
        assert_eq!(ALGORITHM.capabilities & capability::READING_SKILL, 0);
        assert_ne!(ALGORITHM.capabilities & capability::STRUCTURED_MODS, 0);
    }
}
