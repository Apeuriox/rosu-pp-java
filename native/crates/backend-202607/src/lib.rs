//! Adapter for the pinned 2026-07 rework, precision fixes, and taiko/catch updates.
...
pub static ALGORITHM: Algorithm = Algorithm {
    ...
    name: "REWORK_20260706",
    version: "rework-20260706-9a073d2",
    details: "Apeuriox/rosu-pp 9a073d2; osu!standard precision/slider fixes plus 2026 taiko and catch updates",
    capabilities: capability::ALL_BASE | capability::READING_SKILL,
};

pub struct Backend202607;

impl Backend for Backend202607 {
...
impl BackendBeatmap for Map202607 {
    fn algorithm(&self) -> &'static Algorithm {
        &ALGORITHM
    }
...
fn convert_difficulty(attrs: DifficultyAttributes, mode: Mode) -> DifficultyResult {
    ...
    match attrs {
        DifficultyAttributes::Osu(a) => {
            ...
            out.reading = a.reading;
            out.reading_difficult_note_count = a.reading_difficult_note_count;
            out.present |= present::READING | present::READING_DIFFICULT_NOTE_COUNT;
...
mod tests {
    ...
    fn metadata_is_stable() {
        assert_eq!(ALGORITHM.id, 20_260_706);
        assert_eq!(ALGORITHM.version, "rework-20260706-9a073d2");
        assert_ne!(ALGORITHM.capabilities & capability::READING_SKILL, 0);
    }
}
